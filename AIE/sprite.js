...
function Frame(x, y, width, height, duration){
	this.x = x;
	this.y = y;
	this.width = width;
	this.height = height;
...
Sprite.prototype.draw = function(c, x, y, w, h, rot ) {
	...
	if( !w ) w = this.animations[this.currentAnimation][this.currentFrame].width;
	if( !h ) h = this.animations[this.currentAnimation][this.currentFrame].height;
	if( !rot ) rot = 0;
	rot = rot * Math.PI / 180
	...
	c.drawImage(this.image, 
		...
		-w/2, //x+this.offsets[this.currentAnimation].x,
		-h/2, //y+this.offsets[this.currentAnimation].y,
		w,
		h);
